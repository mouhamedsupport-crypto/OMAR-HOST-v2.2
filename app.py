...
def create_default_files(server_dir):
    ...
    if not os.path.exists(main_py):
        with open(main_py, 'w', encoding='utf-8') as f:
            f.write('''# MOUHAMED HOST - Default Bot
import time

print("=" * 40)
print("Bot is running on MOUHAMED HOST")
...
def api_create_server():
    ...
    if not username:
        username = f"MOMO_DEV{random.randint(10000, 99999)}"
    ...
    color = request.args.get('color', '#a050ff').strip() or '#a050ff'
...
def login():
    if request.method == 'POST':
        ...
        users = load_users()
        if username == 'GTMOMO' and password == users.get('GTMOMO', {}).get('password'):
            session['user'] = 'GTMOMO'
            session['role'] = 'admin'
...
def server_login(server_id):
    ...
    banned_ips = result.get('banned_ips', []) if isinstance(result, dict) else []
    if not isinstance(banned_ips, list):
        banned_ips = []
    if client_ip in banned_ips:
        return render_template('error.html', error_type="banned", server_link=server_id)
            ...
            for s in servers:
                if isinstance(s, dict) and s.get('server_id') == server_id:
                    if username == uname and password == data.get('password'):
                        # Update last login info + access log (for admin device/IP tracking)
                        users = load_users()
                        for u, d in users.items():
                            if u == uname:
                                for sv in d.get('servers', []):
                                    if sv.get('server_id') == server_id:
                                        sv['last_login'] = str(datetime.now())
                                        sv['last_ip'] = client_ip
                                        access_entry = {
                                            'ip': client_ip,
                                            'user_agent': request.headers.get('User-Agent', 'Unknown'),
                                            'time': str(datetime.now())
                                        }
                                        log_list = sv.get('access_log', [])
                                        if not isinstance(log_list, list):
                                            log_list = []
                                        log_list.append(access_entry)
                                        sv['access_log'] = log_list[-50:]
                                        save_users(users)
                                        break
                                break
...
def server_home(server_id):
    ...
    banned_ips = result.get('banned_ips', []) if isinstance(result, dict) else []
    if isinstance(banned_ips, list) and request.remote_addr in banned_ips:
        session.clear()
        return render_template('error.html', error_type="banned", server_link=server_id)
    
    return render_template('home.html', username=session['user'], current_server=result, now=datetime.now())
...
def admin_dashboard():
    ...
    for uname, data in users.items():
        ...
        # Get IP info for each server's last login
        for s in servers:
            if s.get('last_ip'):
                s['ip_info'] = get_ip_info(s['last_ip'])
            else:
                s['ip_info'] = None
    ...
    return render_template('admin.html', users=user_list, total_servers=total_servers, 
                          total_running=total_running, admin_ip=client_ip, admin_ip_info=ip_info)
...
def create_server():
    ...
    color = (data.get('color') or '#a050ff').strip() or '#a050ff'
    ...
    new_server = {
        ...
        'cpu_limit': cpu_limit, 'color': color, 'rate_limit_exceeded': False, 'stopped_by_user': False,
...
@app.route('/admin/update_server/<server_id>', methods=['POST'])
        ...
        for s in servers:
            if isinstance(s, dict) and s.get('server_id') == server_id:
                if 'cpu_limit' in data:
                    s['cpu_limit'] = int(data['cpu_limit'])
                if 'ram' in data:
                    s['ram'] = data['ram']
                if 'disk' in data:
                    s['disk'] = data['disk']
...
def api_stats(server_id):
    ...
    uptime, cpu, ram, net_in, net_out = "0s", "0%", "0 MB", "0 KB", "0 KB"
...
# ============================================
# FILE MANAGEMENT - NEW APIs
# ============================================

@app.route('/api/archive/<server_id>', methods=['POST'])
def api_archive(server_id):
    """Archive selected files into a zip"""
    data = request.get_json()
                    ...
                    if os.path.isdir(file_path):
                        for root, dirs, files_in_dir in os.walk(file_path):
                            for file_in_dir in files_in_dir:
                                ...
                                arc_name = os.path.relpath(full_path, server_dir)
                                zf.write(full_path, arc_name)
                    else:
                        zf.write(file_path, f)
        return jsonify({'success': True, 'zip_name': zip_name})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500

@app.route('/api/download_zip/<server_id>', methods=['POST'])
def api_download_zip(server_id):
    """Create zip of selected files and return download link"""
    data = request.get_json()
    files = data.get('files', [])
    folder = data.get('folder', '')
    server_dir = get_server_dir(server_id)
    ...
    zip_name = f"download_{uuid.uuid4().hex[:8]}.zip"
    zip_path = os.path.join(server_dir, zip_name)
...
@app.route('/api/download/<server_id>/<zip_name>')
def api_download(server_id, zip_name):
    """Download a zip file"""
    server_dir = get_server_dir(server_id)
    zip_path = os.path.join(server_dir, zip_name)
...
def api_create_zip(server_id):
    """Create zip from selected files"""
    data = request.get_json()
    files = data.get('files', [])
    folder = data.get('folder', '')
    zip_name = data.get('zip_name', f'archive_{datetime.now().strftime("%Y%m%d_%H%M%S")}.zip')
...
# ============================================
# Admin - Access Log & IP Ban
# ============================================

@app.route('/api/access_log/<server_id>')
def api_access_log(server_id):
    if 'user' not in session or session.get('role') != 'admin':
        return jsonify({'error': 'Unauthorized'}), 403
    ...
    log_list = server.get('access_log', [])
    if not isinstance(log_list, list):
        log_list = []
    banned_ips = server.get('banned_ips', [])
    if not isinstance(banned_ips, list):
        banned_ips = []
    ...
    for entry in reversed(log_list):
        ip = entry.get('ip', '')
        info = get_ip_info(ip) if ip else {}
        enriched.append({
            'ip': ip,
...
@app.route('/api/ban_ip/<server_id>', methods=['POST'])
def api_ban_ip(server_id):
    if 'user' not in session or session.get('role') != 'admin':
        return jsonify({'error': 'Unauthorized'}), 403
    ...
    for uname, udata in users.items():
        ...
        for s in servers:
            if isinstance(s, dict) and s.get('server_id') == server_id:
                ...
                s['banned_ips'] = banned
                save_users(users)
                return jsonify({'success': True, 'banned_ips': banned})
    return jsonify({'error': 'Not found'}), 404

@app.route('/api/unban_ip/<server_id>', methods=['POST'])
def api_unban_ip(server_id):
    if 'user' not in session or session.get('role') != 'admin':
        return jsonify({'error': 'Unauthorized'}), 403
    data = request.get_json() or {}
    ip = (data.get('ip') or '').strip()
...
# ============================================
# Auto-Restart Servers On Panel Boot
# ============================================

def restart_running_servers_on_boot():
    """
    When the main panel process starts (or restarts), any bot server that
    was marked as 'running' before shutdown no longer has a real process
    behind its stored PID (the OS process died with the panel). This walks
    every user's servers and relaunches the ones that were left in the
    'running' state, so bots come back online automatically without an
    admin having to start them by hand.
    """
    users = load_users()
    restarted = 0
    for uname, data in users.items():
        ...
        for s in servers:
            ...
            # Skip servers that are already expired/deleted
            ...
            pid, error = run_bot(server_id, main_file, requirements_file)
            if pid:
                s['status'] = 'running'
                s['pid'] = pid
                s['started_at'] = str(datetime.now())
                ...
                threading.Thread(target=monitor_bot, args=(server_id, pid), daemon=True).start()
                restarted += 1
                print(f"  ↻ Auto-restarted server {server_id} ({uname})")
            else:
                s['status'] = 'stopped'
                s['pid'] = None
                print(f"  ✗ Failed to auto-restart server {server_id} ({uname}): {error}")

    save_users(users)
    if restarted:
        print(f"✅ Auto-restarted {restarted} server(s) that were running before startup.")
    else:
        print("ℹ️ No previously-running servers needed to be restarted.")
...
if __name__ == '__main__':
    ...
    print("👤 admin / MOMO")
    print("=" * 50)
    print("🔄 Checking for servers to auto-restart...")
    if os.environ.get('WERKZEUG_RUN_MAIN') == 'true' or not app.debug:
        restart_running_servers_on_boot()
    else:
        print("  (skipping in reloader parent process)")
    print("=" * 50 + "\n")
    app.run(debug=True, host='0.0.0.0', port=26610)